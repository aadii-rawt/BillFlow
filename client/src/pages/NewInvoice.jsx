...
function NewInvoice() {
    ...
    const [invoiceData, setInvoiceData] = useState({
        ...
        customerName: paramData?.customerName || "",
    ...
    const handleLogoUpload = (e) => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                setInvoiceData(prevData => ({ ...prevData, uploadedLogo: reader.result }));
            };
    ...
    const calculateSubtotal = () => {
        return invoiceData.items.reduce((subtotal, item) => subtotal + parseInt(item.amount), 0).toFixed(2);
    };
    // total amount of bill after adding tax if available
    const calculateTotalWithTax = () => {
        const subtotal = parseFloat(calculateSubtotal());
        const taxPercent = parseFloat(invoiceData.tax) || 0; // Ensure tax is a number
    ...
    const handleVendorChange = (data) => {
        ...
        setInvoiceData((prev) => ({
            ...prev, customerName: data?.displayName, customerId: customerId
        }));
        setError("");
    };
    ...
    useEffect(() => {
        if (paramData?.type == "edit") {
            setInvoiceData(paramData?.bill)
        }
    }, [])

    const [searchQuery, setSearchQuery] = useState("");
    const filteredVendors = customers.filter((data) =>
    ...
    return (
        <div className="relative min-h-screen w-full">
            <div className='p-4 border-b flex justify-between items-center '>
                <h1 className='text-2xl flex items-center gap-3 capitalize'> <TbInvoice size={30} /> {paramData?.type} Invoice</h1>
            ...
            <div className="max-w-4xl p-6 pb-60 rounded-md">
                <div className="flex justify-between items-center mb-6">
                    {/* company logo */}
                    <div>
                        {invoiceData?.uploadedLogo ?
                            <div className="w-24 h-24 max-w-60 relative">
                                <img
                                    src={invoiceData?.uploadedLogo}
                                    alt="Uploaded Logo"
                    ...
                    <div className="text-right">
                        ...
                        <div className="border rounded my-2">
                            <input
                                type="text"
                                required
                                placeholder="Invoice Number #29383"
                ...
                <div className="flex justify-between items-end">
                    ...
                    <div className="gap-6 mb-6">
                        <div className=" relative w-[300px]">
                            <label className="block text-gray-500 mb-1">Invoice To <span className="text-red-600">*</span></label>
                            <div className="w-full relative flex items-center justify-between border rounded-md px-3 py-2 focus:outline-none focus:ring focus:ring-blue-300 cursor-pointer" onClick={() => setShowdropdown(!showdropdown)}>
                                <span className="text-gray-500">{invoiceData?.customerName ? invoiceData?.customerName : "Select or Add Customer"}</span>
                                <IoIosArrowDown />
                                {showdropdown &&
                    ...
                    <div className="text-right space-y-2">
                        ...
                        <div className="flex justify-end relative">
                            ...
                            <div className="min-w-20">
                                <input type="text" value={invoiceData?.tax}
                                    onChange={(e) => {
